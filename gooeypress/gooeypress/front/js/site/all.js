	...
	togglers: function() {
		$('.js_toggler').each(function() {
			...
			$(this).on('click', function(e) {
				...
				if ($target.hasClass('js_toggled_visible')) {
					if ($target.attr('data-toggle-allow-overflow-y') === 'true') {
						$target.removeClass('overflow_y_visible');
					}
					
					$target.removeClass('js_toggled_visible');
				}
				else {
					$target.addClass('js_toggled_visible');
					
					if ($target.attr('data-toggle-allow-overflow-y') === 'true') {
						window.setTimeout(function() {
							$target.addClass('overflow_y_visible');
						}, 600);
					}
				}
			});
